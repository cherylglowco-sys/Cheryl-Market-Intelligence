...
async function fetchDominance() {
  try {
    ...
    if (pct.btc != null || pct.eth != null) return { btc: pct.btc ?? null, eth: pct.eth ?? null, source: "CoinGecko" };
  } catch {}

  const fallback = await fetchJson("https://api.coinlore.net/api/global/");
  const global = Array.isArray(fallback) ? fallback[0] : fallback;
  return {
    btc: global?.btc_d != null ? Number(global.btc_d) : null,
    eth: global?.eth_d != null ? Number(global.eth_d) : null,
    source: "CoinLore"
  };
}
